...
function insertGrades() {
  ...
  ).innerHTML = `${calcSemestr1()}`; //sr1
  gradesTable.children[0].appendChild(
    document.createElement("td")
  ).innerHTML = `<p>${calcProponowane1()}</p>`; //(I)
  gradesTable.children[0].appendChild(
    document.createElement("td")
  ).innerHTML = `<p>${calcKoncowe1()}</p>`; //I
  gradesTable.children[0].appendChild(document.createElement("td")).innerHTML =
    "<p style='text-align: center;'>-></p>"; //Oceny biezace
  gradesTable.children[0].appendChild(
    document.createElement("td")
...
function calcSemestr1(): Grade {
  let semestr1Grades: Grades = [];
  ...
  return parseFloat(
    (semestr1Grades.reduce((a, b) => a + b) / semestr1Grades.length).toFixed(2)
  );
}

function calcProponowane1(): Grade {
  let semestr1Proponowane: Grades = [];
  const przedmiotyLibrus = Array.from(gradesTable.children[1].children).filter(
    (e, idx) => idx % 2 === 0
  ) as Array<Element>;
  przedmiotyLibrus.forEach((val, idX) => {
    ...
    if (
      ...
      return;
    semestr1Proponowane.push(
      parseInt(entry.querySelector("span").querySelector("a").innerText)
    );
  });
  return parseFloat(
    (
      semestr1Proponowane.reduce((a, b) => a + b) / semestr1Proponowane.length
    ).toFixed(2)
  );
}

function calcKoncowe1(): Grade {
  let semestr1Koncowe: Grades = [];
  const przedmiotyLibrus = Array.from(gradesTable.children[1].children).filter(
    (e, idx) => idx % 2 === 0
  ) as Array<Element>;
  przedmiotyLibrus.forEach((val, idX) => {
...
function calcSemestr2(): Grade {
  let semestr2Grades: Grades = [];
  ...
  return parseFloat(
    (semestr2Grades.reduce((a, b) => a + b) / semestr2Grades.length).toFixed(2)
  );
}
...
function calcKoncowe2(): Grade {
  ...
  przedmiotyLibrus.forEach((val, idX) => {
    ...
    if (
      ...
      return;
    semestr2Koncowe.push(
      parseInt(entry.querySelector("span").querySelector("a").innerText)
    );
  });
  console.log(semestr2Koncowe);
  return parseFloat(
    (semestr2Koncowe.reduce((a, b) => a + b) / semestr2Koncowe.length).toFixed(
      2
    )
  );
}
